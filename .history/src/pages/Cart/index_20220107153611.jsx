...
const Cart = () => {
  ...
  const updateQuantity = (e, item) => {
    e.preventDefault();
    console.log("cart price", typeof item.price);
    dispatch(add_cart(item));
  };

  // const cartTotal = !_.isEmpty(cart) ? cart.length > 2 ? cart.reduce(function (a, b) {
  //         return a.total + b.total;
  //       }) : cart.at(0).total : 0;
  const cartTotal = cart.reduce((prev, current) => prev + current.total, 0);
