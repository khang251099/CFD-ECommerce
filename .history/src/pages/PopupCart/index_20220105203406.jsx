...
function PopupCart({ close }) {
  ...
  const removeCart = async (e, item) => {
   ...
   item.map((el)=> el.count===0 ?   toast.success("Remove this product from cart popup") :   dispatch(remove_cart(item)); )
    // if (item.count === 0) {
    //   await toast.warning("Do you wanna delete this item from your cart");
    //   dispatch(delete_cart(item));
    // }
  };
